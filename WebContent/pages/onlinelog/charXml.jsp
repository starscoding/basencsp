...
在线时长:{%Value}{numDecimals:0}]]></format>
					</tooltip_settings>
				</bar_series>
			</data_plot_settings>
			<chart_settings>
			...
			<data>
				...
				<c:forEach var="item" items="${charDataList }">
					<point name='${item.xvalue}（${item.username}）' y='${item.yvalue}'>
						<attributes>
							<attribute name="xvalue">${item.xvalue}</attribute>
							<attribute name="username">${item.username}</attribute>
							<attribute name="fullname">${item.fullname}</attribute>
						</attributes>
					</point>
				</c:forEach>
				</series>
