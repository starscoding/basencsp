/**
 * Provides indentation and folder structure markup for a Tree taking into
 * account depth and position within the tree hierarchy.
 * 
 * @private
 */
Ext.define('Ext.ux.tree.TreeColumnWithDisabled', {
	extend : 'Ext.grid.column.Column',
	alias : 'widget.treecolumnwithdisabled',
	...
	checkboxDisabledText : '<input type="button" role="checkbox" class="{0}" disabled = "disabled" {1} />',
	...
	treeRenderer : function(value, metaData, record, rowIdx, colIdx, store,
		...
		var me = this, buf = [], format = Ext.String.format, depth = record
				.getDepth(), treePrefix = me.treePrefix, elbowPrefix = me.elbowPrefix, elbowPrefix2 = elbowPrefix
				+ 'img', expanderCls = me.expanderCls, imgText = me.imgText, checkboxText = me.checkboxText, checkboxDisabledText = me.checkboxDisabledText, formattedValue = me.origRenderer
				...
				if (record.getDepth() === depth) {
					...
					if (record.get('checked') !== null) {
						if (record.get('disabled') != null) {
							...
							buf.unshift(format(checkboxText,
									(treePrefix + 'checkbox')
											+ (record.get('checked') ? ' '
													+ treePrefix
													+ 'checkbox-checked' : ''),
					...
					if (record.isLast()) {
						if (record.isExpandable()) {
							buf.unshift(format(imgText,
									(elbowPrefix + 'end-plus ' + expanderCls
											+ ' ' + elbowPrefix2), blank));
						} else {
							buf.unshift(format(imgText,
									(elbowPrefix + 'end ' + elbowPrefix2),
									blank));
						}
						...
						if (record.isExpandable()) {
							buf
									.unshift(format(
											imgText,
											(elbowPrefix + 'plus '
													+ expanderCls + ' ' + elbowPrefix2),
											blank));
						} else {
							buf.unshift(format(imgText, (treePrefix + 'elbow'
											+ ' ' + elbowPrefix2), blank));
						}
					}
				} else {
					if (record.isLast() || record.getDepth() === 0) {
						buf
								.unshift(format(imgText, (elbowPrefix
												+ 'empty ' + elbowPrefix2),
										blank));
					} else if (record.getDepth() !== 0) {
						buf.unshift(format(imgText,
								(elbowPrefix + 'line ' + elbowPrefix2), blank));
					}
				}
			}
